...
map_surface/map_geojson:      { surfaceKind: "map_surface", engineKind: "map_geojson", version: "1", surface: {projection?, basemapStyle?, center?, zoom?, bounds?}, display: {surfaceRole?, showLegend?, showLabels?, showInstructionsPanel?, allowLayerToggle?}, state: {sourceId, activeLayerIds?, selectedFeatureIds?, markerCoordinate?, drawnPath?, labelAssignments?, timelineYear?}, interaction: {mode, submissionMode?, allowReset?, resetPolicy?, attemptPolicy?, selectionBehavior?}, feedback: {mode?, displayMode?}, layers: [{id, sourceId, featureIds?, labelField?, visible?, stylePreset?}], evaluation: {acceptedFeatureIds?, featureSelectionMode?, requiredCount?, markerTarget?, expectedPath?, labelTargets?, minimumCoverage?}, annotations: {legendTitle?, guidedPrompts?, callouts?, teacherNotes?} } For maps, `surface.basemapStyle` must be only "none", "light", "terrain", or "satellite"; political/physical/historical/climate/route/custom belong only in `layers[].stylePreset`.
```
...
20. Only use `image`, `label_map`, or `hotspot_select` when the request includes a real http(s) image URL or asset reference to use. Lesson draft `visual_aids[].url` values count as valid request-provided image URLs. Never invent placeholder, local, symbolic, or guessed image URLs.
