...
class BoundedPlanGenerateSkill(StructuredOutputSkill):
    ...
    def build_user_prompt(self, payload: BoundedPlanGenerationRequest, context) -> str:
        lines = [
            ...
            f"Original title candidate: {payload.titleCandidate or 'None provided'}",
            "",
            "Assumptions:",
        ...
        lines.extend(
            [
                "",
                "Generate the smallest durable planning object that fits this source classification and chosen horizon.",
                ...
                "If the source is comprehensive, generate only the bounded opening implied by the entry strategy and chosen horizon.",
                "If the entry strategy is `explicit_range`, stay inside that range.",
                "Keep later continuity possible by respecting `continuationMode`, but do not build future continuation beyond this bounded plan.",
