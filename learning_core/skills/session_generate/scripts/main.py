    ...
    def build_user_prompt(self, payload: SessionPlanGenerationRequest, context) -> str:
        ...
        lines.extend(
            [
                ...
                "",
                "Materials rules:",
                "- required_materials is the parent-facing gather-before-teaching list.",
                "- Include only physical, printed, or digital things the adult should gather before the lesson.",
                "- Do not list vocabulary words, lesson titles, curriculum titles, skill names, abstract concepts, or content-anchor labels as materials.",
                "- Treat the input Materials available list as hints only; clean out non-material words before producing the final list.",
                "- Prefer common household or kid-friendly items when possible, such as small toys, picture cards, paper, crayons, sticky notes, counters, blocks, cups, or a cloth, but only when the block actually uses them.",
                "- Every required material must include name, quantity, category, required, why_needed, used_in_blocks, and easy_substitutes.",
                "- used_in_blocks must contain block titles from this lesson where the material is actually used.",
                "- Blocks that use materials must name them in materials_needed using the same plain names from required_materials.",
                "- Top-level materials is only a compatibility summary copied from required_materials[].name.",
                "- If no supplies are needed, return empty arrays for required_materials, materials, and block materials_needed.",
                "- If any material is required, include an adaptation with trigger if_materials_missing and a concrete fallback path.",
                "",
